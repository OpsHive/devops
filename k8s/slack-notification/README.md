# install slack notification from kubernetes cluster
## step1

1. create user-bot and token which sends k8s notification over channel

use this link to create user-bot 

login as admin and open this link https://my.slack.com/services/new/bot
now you can give bot name and generate token form it which we will use in kubewatch-values.yaml

 
2. create a channel and add you bot user

## step 2

### install kubewatch over cluster
...
```
search for kubewatch helm charts

```
helm search repo kubewatch 

```
before install we can view value.yaml of our kubewatch helm chart just copy these values into a file

```
helm show values bitnami/kubewatch > kubewatch-values.yaml

```

you can edit kubewatch-values.yaml as your requirements. i am just going to edit some parts like slack channel name and bot user token which we have created in first step 
...
slack:
  ...
  ##
  token: "xoxb-1465194189061-2409384990388-hG0yCuwhE6mZZMtgLoJY1DEs"
...
msteams:
  enabled: false
  webhookurl: ""## @param webhook.enabled Enable Webhook notifications
...
install your kubewatch over cluster with helm


```
helm install mykubewatch bitnami/kubewatch --values kubewatch-values.yaml


```


after successfull creatation you will recived slack notifications 
